...
{	
	...
	while(ptrB != NULL)
	{
		if (ptrB -> next == NULL)//check b has a next
		{
			return 0;
		}
		if (ptrA->next == NULL)//check a has a next
		{	

			return 0;
		}
		ptrA = ptrA->next;//increment a
		
		ptrB = ptrB->next;//increment b

		if(ptrB->next == NULL)//check b has another next
		{
			return 0;
		}
		...
		if(ptrA == ptrB)//check if they are the same
		{
			return 1;
		}
	}
